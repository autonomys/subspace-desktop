...
#[tauri::command]
pub(crate) fn winreg_get(sub_key: String, value: String) -> String {
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);
    let subkey = hkcu.open_subkey(sub_key).unwrap();
    let val = subkey.get_value(value);
    let result: String = match val {
        Ok(content) => {
...
#[tauri::command]
pub(crate) fn winreg_set(sub_key: String, set_key: String, value: String) -> String {
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);
    // let subkey = hkcu.open_subkey(sub_key).unwrap();
...
#[tauri::command]
pub(crate) fn winreg_delete(sub_key: String, set_key: String) -> String {
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);
    let (subkey, _disp) = hkcu.create_subkey(&sub_key).unwrap();
