...
describe('Config', () => {
  ...
  it('init method should create directory and config file if there is none', async () => {
    ...
    tauriInvokerMock.readConfig = jest.fn().mockRejectedValue(new Error);
  ...
  it('validate method should return "false" if config file is empty', async () => {
    ...
    tauriInvokerMock.readConfig = jest.fn().mockResolvedValue(emptyConfig);
  ...
  it('remove method should remove config file using tauri.fs method', async () => {
    const config = new Config(params);
    const removeFileSpy = jest.spyOn(tauriInvokerMock, 'removeLinuxAutoLaunchFile');
    await config.remove();
  ...
  it('readConfigFile method should read config file and return it as an object', async () => {
    const config = new Config(params);
    tauriInvokerMock.readConfig = jest.fn().mockResolvedValue(JSON.stringify(configFileMock));
    const result = await config.readConfigFile();

    expect(tauriInvokerMock.readConfig).toHaveBeenCalledWith(configFullPath);
